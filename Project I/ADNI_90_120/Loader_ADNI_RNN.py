...
def process_single(Data: np.ndarray, Label: np.ndarray):
    ...
    # data_2 = min_max_scaler.fit_transform(data_2[:, 0:])
    #
    # data = np.corrcoef(data)
    # data_2 = np.cov(data_2)
    #
    # data = np.tril(data)
    # data_2 = np.tril(data_2)
    #
    # len = data.shape[0]
    # new_data = []
    # for i in range(len):
...
def get_dataloader(Datas, Labels, batch_size=11, shuffle=True, drop_last=False):
...
def YiTiaoLong():
    ...
    Datas, Labels = load('', 1, binary=True)
...
def RNN_YiTiaoLong(test_size=0.1):
    Datas, Labels = load('', 1, binary=True)  # 从文件夹中取数据
